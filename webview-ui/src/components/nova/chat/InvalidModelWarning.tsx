...
import { isAlphaPeriodEndedError, isModelNotAllowedForTeamError } from "@roo/nova/errorUtils"
...
function warningText(innerMessage?: InnerMessage) {
	...
	if (isAlphaPeriodEndedError(innerMessage?.error)) {
		return i18next.t("novacode:invalidModel.alphaPeriodEnded", { model: unavailableModel })
	}
	if (isModelNotAllowedForTeamError(innerMessage?.error)) {
		return i18next.t("novacode:invalidModel.notAllowedForTeam", { model: unavailableModel })
	}
	return i18next.t("novacode:invalidModel.modelUnavailable", { model: unavailableModel })
}
				...
				<>
					<VSCodeButton
						...
						{canChangeToDefaultModel
							? t("novacode:invalidModel.continueWith", {
									model: providerModels[defaultModelId]?.displayName ?? defaultModelId,
								})
							: t("novacode:invalidModel.continue")}
					</VSCodeButton>
