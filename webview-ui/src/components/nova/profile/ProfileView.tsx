...
import NovaCodeAuth from "../common/NovaCodeAuth"
...
const ProfileView: React.FC<ProfileViewProps> = ({ onDone }) => {
	...
	const organizationId = apiConfiguration?.novacodeOrganizationId

	useEffect(() => {
		vscode.postMessage({ type: "fetchProfileDataRequest" })
		vscode.postMessage({ type: "fetchBalanceDataRequest" })
	}, [apiConfiguration?.novacodeToken, organizationId])
		...
		const handleMessage = (event: MessageEvent<WebviewMessage>) => {
			...
			if (message.type === "profileDataResponse") {
				...
				if (payload.success) {
					...
					setBalance(((payload.data as any)?.balance as number) || 0) // novacode_change
				} else {
	...
	return (
		<Tab>
			<TabHeader className="flex justify-between items-center">
				<h3 className="text-vscode-foreground m-0">{t("novacode:profile.title")}</h3>
							...
							<div className="flex flex-col pr-3 h-full">
								...
								<div className="w-full mt-2">
									{organizationId ? (
										<VSCodeButtonLink
											...
											appearance="secondary"
											className="w-full">
											{t("novacode:profile.detailedUsage")}
										</VSCodeButtonLink>
									) : (
										(profileData.organizations?.length ?? 0) === 0 && (
											<VSCodeButtonLink
												...
												appearance="primary"
												className="w-full">
												{t("novacode:profile.createOrganization")}
											</VSCodeButtonLink>
										)
									)}
								</div>
								...
								<div className="w-full flex flex-col items-center">
									<div className="text-sm text-[var(--vscode-descriptionForeground)] mb-3">
										{t("novacode:profile.currentBalance")}
									</div>

									<div className="text-4xl font-bold text-[var(--vscode-foreground)] mb-6 flex items-center gap-2">
										{isLoadingBalance ? (
											<div className="text-[var(--vscode-descriptionForeground)]">
												{t("novacode:profile.loading")}
											</div>
										) : (
											balance && (
												<>
										...
										<div className="w-full mt-8">
														...
														{pkg.popular && (
															<div className="absolute -top-2 left-1/2 transform -translate-x-1/2">
																<span className="bg-[var(--vscode-button-background)] text-[var(--vscode-button-foreground)] text-xs px-2 py-1 rounded-full font-medium">
																	{t("novacode:profile.shop.popular")}
																</span>
															</div>
														)}
											...
											<div className="text-center">
												<VSCodeButtonLink
													...
													appearance="secondary"
													className="text-sm">
													{t("novacode:profile.shop.viewAll")}
												</VSCodeButtonLink>
											</div>
										</div>
									)}
