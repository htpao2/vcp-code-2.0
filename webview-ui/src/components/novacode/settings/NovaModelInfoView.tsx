			...
			<table className="w-full text-sm">
				<thead>
					<tr className="border-b border-vscode-widget-border bg-vscode-editor-background">
						<th className={thClass}>{t("novacode:settings.modelInfo.table.provider")}</th>
						<StandardTooltip content={t("novacode:settings.modelInfo.table.context")}>
							<th className={thClass}>Context</th>
						</StandardTooltip>
						<StandardTooltip content={t("novacode:settings.modelInfo.table.inputPrice")}>
							<th className={thClass}>Input</th>
						</StandardTooltip>
						<StandardTooltip content={t("novacode:settings.modelInfo.table.outputPrice")}>
							<th className={thClass}>Output</th>
						</StandardTooltip>
						<StandardTooltip content={t("novacode:settings.modelInfo.table.cacheReadPrice")}>
							<th className={thClass}>C Read</th>
						</StandardTooltip>
						<StandardTooltip content={t("novacode:settings.modelInfo.table.cacheWritePrice")}>
...
export const NovaModelInfoView = ({
	apiConfiguration,
	modelId,
	model,
		...
		queryFn: async () => {
			try {
				const url = getNovaUrlFromToken(
					"https://api.nova.ai/api/modelstats",
	...
	return (
		<>
			...
			<div className="text-sm text-vscode-descriptionForeground">
				...
				/>
				<div>
					<span className="font-medium">{t("novacode:settings.modelInfo.contextWindow")}:</span>{" "}
			...
			{stats && stats.cost && stats.costPerRequest && model.inputPrice && model.outputPrice ? (
				<StandardTooltip content={t("novacode:settings.modelInfo.averageNovaCodeCost")}>
			...
			<Collapsible open={isPricingExpanded} onOpenChange={setIsPricingExpanded}>
				<CollapsibleTrigger className="flex items-center gap-1 w-full cursor-pointer hover:opacity-80 mb-2">
					...
					<span className="font-medium">{t("novacode:settings.modelInfo.providerBreakdown")}</span>
				</CollapsibleTrigger>
