...
vi.mock("@/hooks/useKeybindings", () => ({
	useKeybindings: () => ({
		"nova-code.autocomplete.generateSuggestions": "Cmd+Shift+G",
	}),
}))
...
vi.mock("@/context/ExtensionStateContext", () => ({
	useExtensionState: () => ({
		novaCodeWrapperProperties: undefined,
	}),
}))
...
describe("AutocompleteServiceSettingsView", () => {
	...
	it("renders basic trigger settings", () => {
		...
		expect(screen.getByText(/novacode:autocomplete.settings.codeEditorSuggestions/)).toBeInTheDocument()
		expect(screen.getByText(/novacode:autocomplete.settings.enableAutoTrigger.label/)).toBeInTheDocument()
	})
	...
	it("toggles auto trigger checkbox correctly", () => {
		...
		const checkboxLabel = screen
			.getByText(/novacode:autocomplete.settings.enableAutoTrigger.label/)
			.closest("label")
	...
	it("toggles smart inline task keybinding checkbox correctly", () => {
		...
		const checkboxLabel = screen
			.getByText(/novacode:autocomplete.settings.enableSmartInlineTaskKeybinding.label/)
			.closest("label")
		const checkbox = checkboxLabel?.querySelector('input[type="checkbox"]') as HTMLInputElement
	...
	it("toggles chat autocomplete checkbox correctly", () => {
		...
		const checkboxLabel = screen
			.getByText(/novacode:autocomplete.settings.enableChatAutocomplete.label/)
			.closest("label")
		const checkbox = checkboxLabel?.querySelector('input[type="checkbox"]') as HTMLInputElement
	...
	it("displays provider and model information when available", () => {
		...
		expect(screen.getByText(/novacode:autocomplete.settings.provider/)).toBeInTheDocument()
	...
	it("displays error message when provider and model are not configured", () => {
		...
		expect(screen.getByText(/novacode:autocomplete.settings.noModelConfigured.title/)).toBeInTheDocument()
	})
	...
	it("displays no credits message when novacode profile exists but has no balance", () => {
		renderComponent({
			ghostServiceSettings: {
				...defaultAutocompleteServiceSettings,
				provider: undefined,
				model: undefined,
				hasNovacodeProfileWithNoBalance: true,
			},
		})

		expect(screen.getByText(/novacode:autocomplete.settings.noCredits.title/)).toBeInTheDocument()
		expect(screen.getByText(/novacode:autocomplete.settings.noCredits.description/)).toBeInTheDocument()
		expect(screen.getByText(/novacode:autocomplete.settings.noCredits.buyCredits/)).toBeInTheDocument()
	})

	it("displays provider and model info even when hasNovacodeProfileWithNoBalance is true but model is configured", () => {
		renderComponent({
			ghostServiceSettings: {
				...defaultAutocompleteServiceSettings,
				provider: "openrouter",
		...
		expect(screen.getByText(/openrouter/)).toBeInTheDocument()
		expect(screen.getByText(/openai\/gpt-4o-mini/)).toBeInTheDocument()
		expect(screen.queryByText(/novacode:autocomplete.settings.noCredits.title/)).not.toBeInTheDocument()
	})
	...
	describe("snooze status refresh", () => {
		it("updates snooze status when timer fires and snooze expires", () => {
			...
			expect(screen.getByText(/novacode:autocomplete.settings.snooze.currentlySnoozed/)).toBeInTheDocument()
			...
			expect(screen.getByText(/novacode:autocomplete.settings.snooze.button/)).toBeInTheDocument()
			expect(screen.queryByText(/novacode:autocomplete.settings.snooze.currentlySnoozed/)).not.toBeInTheDocument()
		})
