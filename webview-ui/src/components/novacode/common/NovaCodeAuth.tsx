...
const NovaCodeAuth: React.FC<NovaCodeAuthProps> = ({ onManualConfigClick, onLoginClick, className = "" }) => {
	...
	return (
		<div className={`flex flex-col items-center ${className}`}>
			...
			<h2 className="m-0 p-0 mb-4">{t("novacode:welcome.greeting")}</h2>
			<p className="text-center mb-2">{t("novacode:welcome.introText1")}</p>
			<p className="text-center mb-2">{t("novacode:welcome.introText2")}</p>
			<p className="text-center mb-5">{t("novacode:welcome.introText3")}</p>

			<div className="w-full flex flex-col gap-5">
				<ButtonPrimary onClick={handleStartDeviceAuth}>{t("novacode:welcome.ctaButton")}</ButtonPrimary>

				{!!onManualConfigClick && (
					<ButtonSecondary onClick={() => onManualConfigClick && onManualConfigClick()}>
						{t("novacode:welcome.manualModeButton")}
					</ButtonSecondary>
				)}
			</div>
		</div>
