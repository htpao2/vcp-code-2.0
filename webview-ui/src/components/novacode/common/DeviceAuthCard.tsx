...
import { generateQRCode } from "@/utils/nova/qrcode"
	...
	return (
		<div className="flex flex-col items-center gap-4 p-6 bg-vscode-sideBar-background rounded">
			...
			<div className="flex items-center gap-2">
				...
				<span className="text-vscode-foreground">{t("novacode:deviceAuth.initiating")}</span>
			</div>
		</div>
	)
}
...
const SuccessState: React.FC<SuccessStateProps> = ({ userEmail }) => {
	...
	return (
		<div className="flex flex-col items-center gap-4 p-6 bg-vscode-sideBar-background rounded">
			<Logo />
			<h3 className="text-lg font-semibold text-vscode-foreground">{t("novacode:deviceAuth.success")}</h3>
			{userEmail && (
				<p className="text-sm text-vscode-descriptionForeground">
					{t("novacode:deviceAuth.authenticatedAs", { email: userEmail })}
				</p>
			)}
		</div>
	)
...
const ErrorState: React.FC<ErrorStateProps> = ({ error, onRetry }) => {
	...
	return (
		<div className="flex flex-col items-center gap-4 p-6 bg-vscode-sideBar-background rounded">
			<Logo />
			<h3 className="text-lg font-semibold text-vscode-foreground">{t("novacode:deviceAuth.error")}</h3>
			<p className="text-sm text-vscode-descriptionForeground text-center">
				{error || t("novacode:deviceAuth.unknownError")}
			</p>
			<ButtonPrimary onClick={onRetry}>{t("novacode:deviceAuth.retry")}</ButtonPrimary>
		</div>
	)
}
...
const CancelledState: React.FC<CancelledStateProps> = ({ onRetry }) => {
	...
	return (
		<div className="flex flex-col items-center gap-4 p-6 bg-vscode-sideBar-background rounded">
			<Logo />
			<h3 className="text-lg font-semibold text-vscode-foreground">{t("novacode:deviceAuth.cancelled")}</h3>
			<ButtonPrimary onClick={onRetry}>{t("novacode:deviceAuth.tryAgain")}</ButtonPrimary>
		</div>
	)
}
	...
	return (
		<div className="flex flex-col gap-2 bg-vscode-sideBar-background rounded">
			<h3 className="text-lg font-semibold text-vscode-foreground text-center">
				{t("novacode:deviceAuth.title")}
			</h3>

			{/* Step 1: URL Section */}
			<div className="flex flex-col gap-3">
				<p className="text-sm text-vscode-descriptionForeground text-center">
					{t("novacode:deviceAuth.step1")}
				</p>
				...
				<div className="flex flex-col gap-2">
					<div className="flex items-center gap-2 p-3 bg-vscode-input-background border border-vscode-input-border rounded">
						...
						<button
							...
							title={t("novacode:deviceAuth.copyUrl")}>
							<span className="codicon codicon-copy text-vscode-foreground"></span>
						</button>
					</div>
					<ButtonPrimary onClick={onOpenBrowser}>{t("novacode:deviceAuth.openBrowser")}</ButtonPrimary>
				</div>

				{/* QR Code Section */}
				{qrCodeDataUrl && (
					<div className="flex flex-col items-center gap-2 mt-2">
						<p className="text-sm text-vscode-descriptionForeground">{t("novacode:deviceAuth.scanQr")}</p>
						<img
							src={qrCodeDataUrl}
							alt="QR Code"
			...
			<div className="flex flex-col gap-3">
				...
				<div className="flex items-center justify-center gap-2">
					...
					<span className="text-sm text-vscode-descriptionForeground">
						{t("novacode:deviceAuth.timeRemaining", { time: formatTime(timeRemaining) })}
					</span>
				</div>
			...
			<div className="w-full flex flex-col">
				<ButtonSecondary onClick={onCancel}>{t("novacode:deviceAuth.cancel")}</ButtonSecondary>
			</div>
		</div>
	)
}
