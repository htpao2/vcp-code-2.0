...
export const OrganizationSelector = ({ className, showLabel = false }: { className?: string; showLabel?: boolean }) => {
	...
	const selectedOrg = organizations.find((o) => o.id === apiConfiguration?.novacodeOrganizationId)
	...
	useEffect(() => {
		if (!apiConfiguration?.novacodeToken) return
	...
	const setSelectedOrganization = (organization: UserOrganizationWithApiKey | null) => {
		if (organization === null) {
			...
			vscode.postMessage({
				...
				values: {
					apiKey: apiConfiguration?.novacodeToken,
				},
			})
		} else {
			vscode.postMessage({
				...
				apiConfiguration: {
					...apiConfiguration,
					novacodeOrganizationId: organization.id,
				},
			})
			vscode.postMessage({
				type: "fetchBalanceDataRequest",
	...
	return (
		<div className={className}>
			{showLabel && (
				<div>
					<label className="block font-medium mb-2">{t("novacode:profile.organization")}</label>
				</div>
			)}
			<div className="relative" ref={containerRef}>
				<button
					...
					<span className="truncate">{selectedOrg ? selectedOrg.name : t("novacode:profile.personal")}</span>
				...
				{isOpen && (
					<div className="absolute z-20 mt-1 right-0 w-max min-w-full max-h-60 overflow-auto rounded border border-[var(--vscode-dropdown-border)] bg-[var(--vscode-dropdown-background)] shadow">
						<div role="listbox" aria-label={t("novacode:profile.organization")}>
							<button
								type="button"
								role="option"
								...
								<span className="truncate">{t("novacode:profile.personal")}</span>
							</button>
