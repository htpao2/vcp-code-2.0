...
import { getExtension } from "@/utils/nova/path-webview"
import { allowedExtensions } from "@roo/nova/rules"
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		if (filename.trim()) {
			...
			if (extension !== "" && !allowedExtensions.includes(extension)) {
				setError(t("novacode:rules.validation.invalidFileExtension"))
				return
			}
				...
				{isExpanded ? (
					<form onSubmit={handleSubmit} className="flex flex-1 items-center">
						<input
							...
							placeholder={
								ruleType === "workflow"
									? t("novacode:rules.placeholders.workflowName")
									: t("novacode:rules.placeholders.ruleName")
							}
							value={filename}
					...
					<>
						<span className="flex-1 text-[var(--vscode-descriptionForeground)] bg-[var(--vscode-input-background)] italic text-xs">
							{ruleType === "workflow"
								? t("novacode:rules.newFile.newWorkflowFile")
								: t("novacode:rules.newFile.newRuleFile")}
						</span>
						<div className="flex items-center ml-2 space-x-2">
							<VSCodeButton
								appearance="icon"
