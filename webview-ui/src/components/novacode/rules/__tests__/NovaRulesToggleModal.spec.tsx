...
import NovaRulesToggleModal from "../NovaRulesToggleModal"
...
vi.mock("../NovaRulesWorkflowsView", () => ({
	__esModule: true,
	default: ({ type }: { type: "rule" | "workflow" }) => <div data-testid={`rules-workflows-${type}`} />,
}))

describe("NovaRulesToggleModal", () => {
	it("renders Modes, MCP, Rules, and Workflows as tabs", () => {
		render(<NovaRulesToggleModal />)

		fireEvent.click(screen.getByLabelText("novacode:rules.ariaLabel"))

		expect(screen.getByText("settings:sections.modes")).toBeInTheDocument()
		expect(screen.getByText("novacode:settings.sections.mcp")).toBeInTheDocument()
		expect(screen.getByText("novacode:rules.tabs.rules")).toBeInTheDocument()
		expect(screen.getByText("novacode:rules.tabs.workflows")).toBeInTheDocument()

		fireEvent.click(screen.getByText("settings:sections.modes"))
		expect(screen.getByTestId("modes-view")).toBeInTheDocument()

		fireEvent.click(screen.getByText("novacode:settings.sections.mcp"))
		expect(screen.getByTestId("mcp-view")).toBeInTheDocument()

		fireEvent.click(screen.getByText("novacode:rules.tabs.rules"))
		expect(screen.getByTestId("rules-workflows-rule")).toBeInTheDocument()

		fireEvent.click(screen.getByText("novacode:rules.tabs.workflows"))
