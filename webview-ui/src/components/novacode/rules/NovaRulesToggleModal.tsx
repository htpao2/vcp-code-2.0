...
import NovaRulesWorkflowsView from "./NovaRulesWorkflowsView"
...
const NovaRulesToggleModal: React.FC = () => {
	const { t } = useTranslation()
	// novacode_change - tooltip now reflects Agent Behaviour scope
	const agentBehaviourTypes = [
		t("novacode:rules.agentBehaviourTypes.rules"),
		t("novacode:rules.agentBehaviourTypes.workflows"),
		t("novacode:rules.agentBehaviourTypes.mcps"),
		t("novacode:rules.agentBehaviourTypes.modes"),
		t("novacode:rules.agentBehaviourTypes.skills"),
	].join(", ")
		...
		<div ref={modalRef}>
					...
					<Tooltip open={isVisible ? false : undefined}>
						<TooltipTrigger asChild>
							<BottomButton
								...
								ariaLabel={t("novacode:rules.ariaLabel")}
								onClick={() => setIsVisible(!isVisible)}
							/>
						</TooltipTrigger>
						<TooltipContent>{t("novacode:rules.tooltip", { types: agentBehaviourTypes })}</TooltipContent>
					</Tooltip>
				</TooltipProvider>
			</div>
			...
			{isVisible && (
				<div
					...
					<div
						data-testid="nova-rules-toggle-modal-content"
						style={{
							flex: 1,
							overflowY: "auto",
							overflowX: "hidden",
						...
						{currentView === "rule" && <NovaRulesWorkflowsView type="rule" />}
						{currentView === "workflow" && <NovaRulesWorkflowsView type="workflow" />}
...
export default NovaRulesToggleModal
