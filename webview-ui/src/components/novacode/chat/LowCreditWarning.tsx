...
export const LowCreditWarning = ({ message }: LowCreditWarningProps) => {
	...
	const handleSwitchToFreeModel = () => {
		...
		vscode.postMessage({
			...
			apiConfiguration: {
				...apiConfiguration,
				novacodeModel: data.defaultFreeModel,
			},
		})
		...
		<>
			...
			<div
				...
				<div className="flex justify-between items-center">
					{t("novacode:lowCreditWarning.lowBalance")}
					<RetryIconButton
						onClick={() => {
							handleRetry()
						}}
				...
				<VSCodeButton
					...
					{t("novacode:lowCreditWarning.addCredit")}
				</VSCodeButton>
				{apiConfiguration?.novacodeModel !== data.defaultFreeModel &&
					(data.defaultFreeModel ? (
						<VSCodeButton
							className="p-1 w-full rounded mt-1"
							appearance="primary"
							onClick={handleSwitchToFreeModel}>
							{t("novacode:lowCreditWarning.switchToFreeModel")}
						</VSCodeButton>
					) : (
