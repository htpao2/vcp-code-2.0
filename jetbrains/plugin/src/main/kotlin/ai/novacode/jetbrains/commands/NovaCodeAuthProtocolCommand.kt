...
package ai.novacode.jetbrains.commands
...
/**
 * JetBrains Protocol Command for handling Nova Code authentication URLs
 *
 * Handles URLs in the format: jetbrains://idea/ai.novacode.jetbrains.auth?token=HERE
 * and forwards them to the VSCode extension via RPC protocol
 */
class NovaCodeAuthProtocolCommand : JBProtocolCommand("ai.novacode.jetbrains.auth") {
    private val logger = Logger.getInstance(NovaCodeAuthProtocolCommand::class.java)
    private val coroutineScope = CoroutineScope(SupervisorJob() + Dispatchers.IO)

    companion object {
        const val COMMAND_ID = "ai.novacode.jetbrains.auth"
    ...
    override suspend fun execute(target: String?, parameters: Map<String, String>, fragment: String?): String? {
        logger.info("Handling Nova Code auth protocol command: target=$target, parameters=$parameters")
        ...
        return try {
            ...
            val errorMsg = "Error handling Nova Code auth protocol command: ${e.message}"
    ...
    private fun forwardTokenToVSCodeExtension(token: String) {
        coroutineScope.launch {
            try {
                ...
                val vscodeUriString = "vscode://novacode.nova-code/novacode?token=$token"
