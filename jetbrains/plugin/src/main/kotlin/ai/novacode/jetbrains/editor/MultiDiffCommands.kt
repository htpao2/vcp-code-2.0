    ...
    suspend fun vscode_changes(title: String, changes: List<List<Map<String, Any?>>>): Any? {
        ...
        try {
            ...
            for ((index, changeTriplet) in changes.withIndex()) {
                ...
                val originalFileUri = ai.novacode.jetbrains.editor.createURI(changeTriplet[0])
                val beforeContentUri = ai.novacode.jetbrains.editor.createURI(changeTriplet[1])
                val afterContentUri = ai.novacode.jetbrains.editor.createURI(changeTriplet[2])
