...
import ai.novacode.jetbrains.ipc.proxy.LazyPromise
...
class CommitMessageService {
    ...
    suspend fun generateCommitMessage(
        ...
        return try {
            ...
            if (proxy == null) {
                ...
                return Result.Error(I18n.t("novacode:commitMessage.errors.connectionFailed"))
            }
            ...
            Result.Error(I18n.t("novacode:commitMessage.errors.timeout"))
        } catch (e: Exception) {
            logger.error("Commit message generation failed", e)
            Result.Error(I18n.t("novacode:commitMessage.generationFailed", mapOf("errorMessage" to (e.message ?: I18n.t("novacode:commitMessage.error.unknown")))))
        }
    }
    ...
    private fun processCommandResult(result: Any?): Result {
        ...
        if (result !is Map<*, *>) {
            ...
            return Result.Error(I18n.t("novacode:commitMessage.errors.invalidResponse"))
        }
        ...
        if (error != null) {
            ...
            return Result.Error(I18n.t("novacode:commitMessage.generationFailed", mapOf("errorMessage" to error)))
        }

        // Handle missing message
        if (message == null) {
            logger.warn("Received response without message or error field")
            return Result.Error(I18n.t("novacode:commitMessage.errors.missingMessage"))
        }
