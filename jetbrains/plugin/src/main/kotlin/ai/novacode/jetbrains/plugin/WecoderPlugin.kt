package ai.novacode.jetbrains.plugin

import ai.novacode.jetbrains.core.ExtensionProcessManager
import ai.novacode.jetbrains.core.ExtensionSocketServer
import ai.novacode.jetbrains.core.ExtensionUnixDomainSocketServer
import ai.novacode.jetbrains.core.ISocketServer
import ai.novacode.jetbrains.core.ServiceProxyRegistry
import ai.novacode.jetbrains.monitoring.ScopeRegistry
import ai.novacode.jetbrains.monitoring.ThreadMonitor
import ai.novacode.jetbrains.monitoring.DisposableTracker
import ai.novacode.jetbrains.util.ExtensionUtils
import ai.novacode.jetbrains.util.PluginConstants
import ai.novacode.jetbrains.util.PluginResourceUtil
import ai.novacode.jetbrains.webview.WebViewManager
import ai.novacode.jetbrains.workspace.WorkspaceFileChangeManager
...
class WecoderPlugin : StartupActivity.DumbAware {
    ...
    override fun runActivity(project: Project) {
        ...
        LOG.info(
            "Initializing Nova Code plugin for project: ${project.name}, " +
        ...
        try {
            ...
            Disposer.register(
                project,
                Disposable {
                    LOG.info("Disposing Nova Code plugin for project: ${project.name}")
                    pluginService.dispose()
                    // SystemObjectProvider is now project-scoped and will be disposed automatically
                },
            )

            LOG.info("Nova Code plugin initialized successfully for project: ${project.name}")
        } catch (e: Exception) {
            LOG.error("Failed to initialize Nova Code plugin", e)
        }
    }
}
...
class WecoderPluginService(private var currentProject: Project) : Disposable {
    ...
    private val boundedIODispatcher = Executors.newFixedThreadPool(
        ...
        { r -> Thread(r, "NovaCode-IO").apply { isDaemon = true } }
    ...
    companion object {
        ...
        init {
            try {
                ...
                val configStream: InputStream? = WecoderPluginService::class.java.getResourceAsStream("/ai/novacode/jetbrains/plugin/config/plugin.properties")
