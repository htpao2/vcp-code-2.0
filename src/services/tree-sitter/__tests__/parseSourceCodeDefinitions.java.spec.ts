...
describe("parseSourceCodeDefinitionsForFile with Java", () => {
	...
	it("should parse method declarations", () => {
		...
		expect(parseResult).toMatch(/\d+--\d+ \|\s*public void testInterfaceMethod\(String message, T data\) {/) // novacode_change
		expect(parseResult).toMatch(/\d+--\d+ \|\s*default String testInterfaceDefaultMethod\(/)
		expect(parseResult).toMatch(/\d+--\d+ \|\s*void testMultipleAnnotationMethod\(String message, T data\) {/) // novacode_change
