import { checkNovacodeBalance } from "./novacode-utils"

describe("checkNovacodeBalance", () => {
	...
	it("should return true when balance is positive", async () => {
		...
		const result = await checkNovacodeBalance(mockToken)
	...
	it("should include organization ID in headers when provided", async () => {
		...
		const result = await checkNovacodeBalance(mockToken, mockOrgId)
		expect(result).toBe(true)
		expect(global.fetch).toHaveBeenCalledWith(
			"https://api.nova.ai/api/profile/balance",
			expect.objectContaining({
				headers: expect.objectContaining({
					Authorization: `Bearer ${mockToken}`,
					"X-NovaCode-OrganizationId": mockOrgId,
				}),
			}),
		)
	})
	...
	it("should not include organization ID in headers when not provided", async () => {
		...
		await checkNovacodeBalance(mockToken)
		...
		expect(headers).not.toHaveProperty("X-NovaCode-OrganizationId")
	})
