...
export type NovacodeConfigProject = z.infer<typeof NovacodeConfigProject>
export const NovacodeConfigProject = z.object({
	id: z.string().optional(),
	managedIndexingEnabled: z.boolean().optional(),
	baseBranch: z.string().optional(),
})

export const NovacodeConfig = z.object({
	project: NovacodeConfigProject.optional(),
})

export type NovacodeConfig = z.infer<typeof NovacodeConfig>
 ...
 *
 * @example
 * normalizeProjectId('https://github.com/Nova-Org/handbook.git') // returns 'handbook'
 * normalizeProjectId('git@github.com:Nova-Org/handbook.git') // returns 'handbook'
 * normalizeProjectId('my-project') // returns 'my-project'
 * normalizeProjectId(undefined) // returns undefined
 */
export function normalizeProjectId(projectId?: NovacodeConfigProject["id"]): string | undefined {
	if (!projectId) {
		return undefined
	}
...
export async function getNovacodeConfig(
	workspaceRoot: string,
	gitRepositoryUrl?: string,
): Promise<NovacodeConfig | null> {
	console.log("getNovacodeConfig", workspaceRoot, gitRepositoryUrl)

	let config = await getNovacodeConfigFile(workspaceRoot)
...
/**
 * Reads the project configuration from .novacode/config.json
 * Note: .novacode/config.jsonc is not supported to avoid bundling issues
 *
 * @param workspaceRoot The root path of the workspace
 * @returns The project configuration or undefined if not found or invalid
 */
export async function getNovacodeConfigFile(workspaceRoot: string): Promise<NovacodeConfig | null> {
	const configPath = path.join(workspaceRoot, ".novacode", "config.json")
	try {
		const content = await fs.readFile(configPath, "utf8")
		const config = NovacodeConfig.parse(JSON.parse(content))
		return config
	} catch (error) {
 ...
 * Priority:
 * 1. .novacode/config.json (project.id) - normalized
...
export async function getProjectId(workspaceRoot: string, gitRepositoryUrl?: string): Promise<string | undefined> {
	...
	const config = await getNovacodeConfigFile(workspaceRoot)
	if (config?.project?.id) {
