...
describe("Ollama Fetcher", () => {
	...
	describe("parseOllamaModel", () => {
		it("should correctly parse Ollama model info", () => {
			...
			expect(parsedModel).toEqual({
				maxTokens: 4096, // novacode_change
				contextWindow: 4096, // novacode_change
				...
				cacheWritesPrice: 0,
				cacheReadsPrice: 0,
				description: "Family: qwen3, Context: 4096, Size: 32.8B", // novacode_change
			})
		})
		...
		// novacode_change: should return model with supportsNativeTools=false when capabilities does not include 'tools'
		it("should return model with supportsNativeTools=false when capabilities does not include 'tools'", () => {
			...
			// novacode_change start: Models without tools capability are still returned (for autocomplete), but marked as not supporting tools
		...
		// novacode_change: should return model with supportsNativeTools=false when capabilities is undefined
		it("should return model with supportsNativeTools=false when capabilities is undefined", () => {
			...
			// novacode_change start: Models without explicit tools capability are still returned (for autocomplete)
			expect(parsedModel).not.toBeNull()
			expect(parsedModel!.supportsNativeTools).toBe(false)
			// novacode_change end
		})

		// novacode_change: should return model with vision but supportsNativeTools=false when no tools capability
		it("should return model with vision but supportsNativeTools=false when no tools capability", () => {
			...
			// novacode_change start: No "tools" capability but model is still returned (for autocomplete/vision tasks)
			expect(parsedModel).not.toBeNull()
	...
	describe("getOllamaModels", () => {
		// novacode_change: should fetch model list from /api/tags and include all models
		...
		// novacode_change: should include models without tools capability but mark supportsNativeTools=false
		it("should include models without tools capability but mark supportsNativeTools=false", async () => {
			...
			// novacode_change start: Model without tools capability is now included (for autocomplete), but marked as not supporting tools
			expect(Object.keys(result).length).toBe(1)
			expect(result[modelName]).toBeDefined()
