...
export class VsCodeLmHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		try {
			...
			throw new Error(
				`Nova Code <Language Model API>: Failed to initialize handler: ${error instanceof Error ? error.message : "Unknown error"}`,
			)
		}
	}
	/**
	...
	async initializeClient(): Promise<void> {
		try {
			...
			if (this.client) {
				console.debug("Nova Code <Language Model API>: Client already initialized")
				return
			}
			// Create a new client instance
			this.client = await this.createClient(this.options.vsCodeLmModelSelector || {})
			console.debug("Nova Code <Language Model API>: Client initialized successfully")
		} catch (error) {
			// Handle errors during client initialization
			const errorMessage = error instanceof Error ? error.message : "Unknown error"
			console.error("Nova Code <Language Model API>: Client initialization failed:", errorMessage)
			throw new Error(`Nova Code <Language Model API>: Failed to initialize client: ${errorMessage}`)
		}
	}
	/**
	...
	async createClient(selector: vscode.LanguageModelChatSelector): Promise<vscode.LanguageModelChat> {
		try {
			...
			const errorMessage = error instanceof Error ? error.message : "Unknown error"
			throw new Error(`Nova Code <Language Model API>: Failed to select model: ${errorMessage}`)
		}
	}
	...
	private async internalCountTokens(text: string | vscode.LanguageModelChatMessage): Promise<number> {
		...
		if (!this.client) {
			console.warn("Nova Code <Language Model API>: No client available for token counting")
			return 0
		}

		if (!this.currentRequestCancellation) {
			console.warn("Nova Code <Language Model API>: No cancellation token available for token counting")
			return 0
		}

		// Validate input
		if (!text) {
			console.debug("Nova Code <Language Model API>: Empty text provided for token counting")
			return 0
		}
		...
		try {
			...
			if (typeof text === "string") {
				...
				if (!text.content || (Array.isArray(text.content) && text.content.length === 0)) {
					console.debug("Nova Code <Language Model API>: Empty chat message content")
					return 0
				}
				...
				console.warn("Nova Code <Language Model API>: Invalid input type for token counting")
				return 0
			}

			// Validate the result
			if (typeof tokenCount !== "number") {
				console.warn("Nova Code <Language Model API>: Non-numeric token count received:", tokenCount)
				return 0
			}

			if (tokenCount < 0) {
				console.warn("Nova Code <Language Model API>: Negative token count received:", tokenCount)
				return 0
			}
			...
			if (error instanceof vscode.CancellationError) {
				console.debug("Nova Code <Language Model API>: Token counting cancelled by user")
				return 0
			}

			const errorMessage = error instanceof Error ? error.message : "Unknown error"
			console.warn("Nova Code <Language Model API>: Token counting failed:", errorMessage)
	...
	private async getClient(): Promise<vscode.LanguageModelChat> {
		if (!this.client) {
			console.debug("Nova Code <Language Model API>: Getting client with options:", {
			...
			try {
				...
				console.debug("Nova Code <Language Model API>: Creating client with selector:", selector)
				this.client = await this.createClient(selector)
			} catch (error) {
				const message = error instanceof Error ? error.message : "Unknown error"
				console.error("Nova Code <Language Model API>: Client creation failed:", message)
				throw new Error(`Nova Code <Language Model API>: Failed to create client: ${message}`)
			}
		}
	...
	override async *createMessage(
		...
		try {
			...
			const requestOptions: vscode.LanguageModelChatRequestOptions = {
				justification: `Nova Code would like to use '${client.name}' from '${client.vendor}', Click 'Allow' to proceed.`,
			}
			...
			for await (const chunk of response.stream) {
				if (chunk instanceof vscode.LanguageModelTextPart) {
					...
					if (typeof chunk.value !== "string") {
						console.warn("Nova Code <Language Model API>: Invalid text part value received:", chunk.value)
						continue
					}
					...
					try {
						...
						if (!chunk.name || typeof chunk.name !== "string") {
							console.warn("Nova Code <Language Model API>: Invalid tool name received:", chunk.name)
							continue
						}

						if (!chunk.callId || typeof chunk.callId !== "string") {
							console.warn("Nova Code <Language Model API>: Invalid tool callId received:", chunk.callId)
							continue
						}

						// Ensure input is a valid object
						if (!chunk.input || typeof chunk.input !== "object") {
							console.warn("Nova Code <Language Model API>: Invalid tool input received:", chunk.input)
							continue
						}

						// Log tool call for debugging
						console.debug("Nova Code <Language Model API>: Processing tool call:", {
							name: chunk.name,
							callId: chunk.callId,
						...
						console.error("Nova Code <Language Model API>: Failed to process tool call:", error)
						// Continue processing other chunks even if one fails
						continue
					}
				} else {
					console.warn("Nova Code <Language Model API>: Unknown chunk type received:", chunk)
				}
			}
			...
			if (error instanceof vscode.CancellationError) {
				throw new Error("Nova Code <Language Model API>: Request cancelled by user")
			}

			if (error instanceof Error) {
				console.error("Nova Code <Language Model API>: Stream error details:", {
				...
				console.error("Nova Code <Language Model API>: Stream error object:", errorDetails)
				throw new Error(`Nova Code <Language Model API>: Response stream error: ${errorDetails}`)
			} else {
				// Fallback for unknown error types
				const errorMessage = String(error)
				console.error("Nova Code <Language Model API>: Unknown stream error:", errorMessage)
				throw new Error(`Nova Code <Language Model API>: Response stream error: ${errorMessage}`)
			}
		}
	}
	...
	override getModel(): { id: string; info: ModelInfo } {
		if (this.client) {
			...
			for (const [prop, value] of Object.entries(requiredProps)) {
				if (!value && value !== 0) {
					console.warn(`Nova Code <Language Model API>: Client missing ${prop} property`)
				}
			}
		...
		console.debug("Nova Code <Language Model API>: No client available, using fallback model info")
