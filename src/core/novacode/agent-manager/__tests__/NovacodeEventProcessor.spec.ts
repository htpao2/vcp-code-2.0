...
import type { NovacodeStreamEvent } from "../CliOutputParser"
...
describe("NovacodeEventProcessor", () => {
	const sessionId = "session-1"

	it("skips user echo before api_req_started", () => {
		const deps = createDeps()
		const processor = new NovacodeEventProcessor(deps)

		const textEvent: NovacodeStreamEvent = {
	...
	it("records api_req_started and then allows subsequent text", () => {
		...
		const apiStarted: NovacodeStreamEvent = {
			streamEventType: "novacode",
	...
	it("calls postStateEvent with ask_followup when followup ask is received", () => {
		...
		const followupEvent: NovacodeStreamEvent = {
			streamEventType: "novacode",
	...
	it("calls postStateEvent with ask_completion_result when say:completion_result is received", () => {
		...
		const completionEvent: NovacodeStreamEvent = {
			streamEventType: "novacode",
	...
	it("calls postStateEvent with ask_completion_result when ask:completion_result is received", () => {
		...
		const completionAskEvent: NovacodeStreamEvent = {
			streamEventType: "novacode",
	...
	it("stops process and marks error on payment_required_prompt (once)", () => {
		...
		const payEvent: NovacodeStreamEvent = {
			streamEventType: "novacode",
	...
	it("renders checkpoints as checkpoint_saved entries with no text", () => {
		...
		const checkpointEvent: NovacodeStreamEvent = {
			streamEventType: "novacode",
			payload: {
				type: "say",
	...
	it("extracts command_output text from metadata.output", () => {
		...
		const cmdOutputEvent: NovacodeStreamEvent = {
			streamEventType: "novacode",
			payload: {
				type: "ask",
				ask: "command_output",
	...
	it("keeps empty partial command_output messages (no placeholder text)", () => {
		...
		const cmdOutputStart: NovacodeStreamEvent = {
			streamEventType: "novacode",
			payload: {
				type: "ask",
				ask: "command_output",
	...
	it("deduplicates command_output messages by executionId (not timestamp)", () => {
		...
		const cmdOutputPending: NovacodeStreamEvent = {
			streamEventType: "novacode",
			payload: {
				type: "ask",
				ask: "command_output",
		...
		const cmdOutputApproved: NovacodeStreamEvent = {
			streamEventType: "novacode",
			payload: {
				type: "ask",
				ask: "command_output",
