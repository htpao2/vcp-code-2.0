...
describe("parallelModeParser", () => {
	...
	describe("parseParallelModeWorktreePath", () => {
		it("parses path from 'Created worktree at:' message", () => {
			const message = "Created worktree at: /tmp/novacode-worktree-add-feature"
			expect(parseParallelModeWorktreePath(message)).toBe("/tmp/novacode-worktree-add-feature")
		})

		it("handles paths with spaces (edge case)", () => {
			const message = "Created worktree at: /var/folders/abc/novacode-worktree-test"
			expect(parseParallelModeWorktreePath(message)).toBe("/var/folders/abc/novacode-worktree-test")
		})
		...
		it("handles Windows-style paths", () => {
			const message = "Created worktree at: C:\\Users\\test\\novacode-worktree-feature"
			expect(parseParallelModeWorktreePath(message)).toBe("C:\\Users\\test\\novacode-worktree-feature")
		})
	})
	...
	describe("buildParallelModeWorktreePath", () => {
		it("uses the OS temp directory with the novacode worktree prefix", () => {
			const branch = "feature-123"
			const result = buildParallelModeWorktreePath(branch)
			expect(result).toContain(`novacode-worktree-${branch}`)
		})
	})
})
