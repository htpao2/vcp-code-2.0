...
describe("SetupScriptService", () => {
	...
	describe("getScriptPath", () => {
		it("returns correct path to setup-script file", () => {
			const expectedPath = path.join(testWorkspacePath, ".novacode", "setup-script")
	...
	describe("createDefaultScript", () => {
		it("creates .novacode directory if it does not exist", async () => {
			vi.mocked(fs.existsSync).mockReturnValue(false)
			...
			expect(fs.promises.mkdir).toHaveBeenCalledWith(
				path.join(testWorkspacePath, ".novacode"),
