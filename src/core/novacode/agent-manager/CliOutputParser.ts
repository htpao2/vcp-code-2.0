 ...
 * CLI Output Parser
 *
 * Parses nd-json output from the novacode CLI, handling ANSI escape codes
...
export interface NovacodePayload extends Record<string, unknown> {
	type?: string
	say?: string
	ask?: string
	content?: string
...
export interface NovacodeStreamEvent {
	streamEventType: "novacode"
	payload: NovacodePayload
	sessionId?: string
}
...
function toStreamEvent(parsed: Record<string, unknown>): StreamEvent | null {
	...
	// Legacy format from CLI stdout: wrap parsed object as novacode payload
	...
	return {
		streamEventType: "novacode",
		payload: parsed as NovacodePayload,
	}
}
