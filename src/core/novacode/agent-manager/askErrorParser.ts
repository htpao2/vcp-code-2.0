import { z } from "zod"
import { NovacodePayload } from "./CliOutputParser"
 ...
 */
export function extractRawText(payload: NovacodePayload | { text?: string; content?: string }): string {
...
export function extractPayloadMessage(payload: NovacodePayload, defaultMessage: string): string {
...
export function extractApiReqFailedMessage(payload: NovacodePayload): { message: string; authError: boolean } {
	const rawText = extractRawText(payload)
