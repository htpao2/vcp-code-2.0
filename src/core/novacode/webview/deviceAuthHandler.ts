...
import { DeviceAuthService } from "../../../services/nova/DeviceAuthService"
...
/**
 * Handles device authorization flow for Nova Code authentication
		...
		try {
			...
			this.deviceAuthService.on("success", async (token: any, userEmail: any) => {
				...
				this.callbacks.showInformationMessage(
					`Nova Code successfully configured! Authenticated as ${userEmail}`,
				)
