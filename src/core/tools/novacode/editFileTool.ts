...
import { FastApplyApiProvider, fastApplyApiProviderSchema, getNovaUrlFromToken } from "@roo-code/types"
import { DEFAULT_HEADERS } from "../../../api/providers/constants"
import { TelemetryService } from "@roo-code/telemetry"
import { type ClineProviderState } from "../../webview/ClineProvider"
import { ClineSayTool } from "../../../shared/ExtensionMessage"
import { X_NOVACODE_ORGANIZATIONID, X_NOVACODE_TASKID, X_NOVACODE_TESTER } from "../../../shared/nova/headers"
...
async function applyFastApplyEdit(
	...
	try {
		...
		const novaTesterSuppressUntil = state.apiConfiguration.novacodeTesterWarningsDisabledUntil
		const novaTesterSuppress =
			novaTesterSuppressUntil && novaTesterSuppressUntil > Date.now() ? { [X_NOVACODE_TESTER]: "SUPPRESS" } : {}
		...
		const client = new OpenAI({
			...
			defaultHeaders: {
				...DEFAULT_HEADERS,
				...(morphConfig.novaCodeOrganizationId
					? { [X_NOVACODE_ORGANIZATIONID]: morphConfig.novaCodeOrganizationId }
					: {}),
				...novaTesterSuppress,
				[X_NOVACODE_TASKID]: cline.taskId,
			},
		})
...
function getFastApplyConfiguration(state: ClineProviderState): FastApplyConfiguration {
	...
	// Priority 2: Use NovaCode provider
	if (apiProvider === "novacode") {
		const token = useCurrentApiConfiguration ? state.apiConfiguration.novacodeToken : state.morphApiKey
		if (!token) {
			return { available: false, error: "No NovaCode token available to use Fast Apply" }
		}
		const url = getNovaUrlFromToken("https://api.nova.ai/api/openrouter/", token)
		...
		return {
			...
			novaCodeOrganizationId: state.apiConfiguration.novacodeOrganizationId,
		}
	}
