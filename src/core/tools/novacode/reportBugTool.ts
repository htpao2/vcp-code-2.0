...
export async function reportBugTool(
	...
	try {
		if (block.partial) {
			...
			const novacodeVersion = getCurrentOrLegacyExtension()?.packageJSON.version || "Unknown"
			...
			const bugReportData = JSON.stringify({
				...
				novacode_version: novacodeVersion,
			})
			...
			if (text || images?.length) {
				...
				try {
					...
					await createAndOpenGitHubIssue("Nova-Org", "novacode", "bug_report.yml", params)
				} catch (error) {
