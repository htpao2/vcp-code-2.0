...
import { allowedExtensions } from "../../shared/nova/rules"
...
export async function getEnabledRules(
	...
	const globalRulesBase = path.join(homedir, GlobalFileNames.novaRules)
	const localRulesBase = path.join(workspacePath, GlobalFileNames.novaRules)
	...
	// Also include mode-specific rules directories (e.g. .novacode/rules-code/)
	if (mode) {
		const globalModeRulesDir = path.join(homedir, ".novacode", `rules-${mode}`)
		const localModeRulesDir = path.join(workspacePath, ".novacode", `rules-${mode}`)
...
function getRuleDirectoryPath(baseDir: string, ruleType: "rule" | "workflow") {
	return ruleType === "workflow"
		...
		: path.join(baseDir, GlobalFileNames.novaRules)
}
...
export async function createRuleFile(
	...
	if (!workspacePath && !isGlobal) {
		vscode.window.showErrorMessage(t("novacode:rules.errors.noWorkspaceFound"))
		return
	}
	...
	if (await fileExistsAtPath(filePath)) {
		vscode.window.showErrorMessage(t("novacode:rules.errors.fileAlreadyExists", { filename }))
		return
	}
...
${t("novacode:rules.templates.workflow.description")}

${t("novacode:rules.templates.workflow.stepsHeader")}

1. ${t("novacode:rules.templates.workflow.step1")}
2. ${t("novacode:rules.templates.workflow.step2")}
`
}
...
${t("novacode:rules.templates.rule.description")}

${t("novacode:rules.templates.rule.guidelinesHeader")}

- ${t("novacode:rules.templates.rule.guideline1")}
- ${t("novacode:rules.templates.rule.guideline2")}
`
}

export async function deleteRuleFile(rulePath: string): Promise<void> {
	const deleteAction = t("novacode:rules.actions.delete")
	const filename = path.basename(rulePath)
	const result = await vscode.window.showWarningMessage(
		t("novacode:rules.actions.confirmDelete", { filename }),
		{ modal: true },
		deleteAction,
	)
	...
	if (result === deleteAction) {
		...
		vscode.window.showInformationMessage(t("novacode:rules.actions.deleted", { filename }))
	}
}
