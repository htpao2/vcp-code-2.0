...
import {
	...
	type ClineMessage,
	openRouterDefaultModelId, // novacode_change: openRouterDefaultModelId
...
vi.mock("vscode", () => ({
	...
	env: {
		uriScheme: "vscode",
		language: "en",
		uiKind: 1, // novacode_change Desktop
...
describe("ClineProvider", () => {
	...
	test("resolveWebviewView sets up webview correctly in development mode even if local server is not running", async () => {
		...
		expect(mockWebviewView.webview.html).toContain(
			// novacode_change: added localhost:3000
	...
	test("postMessageToWebview sends message to webview", async () => {
		...
		const mockState: ExtensionState = {
			...
			apiConfiguration: {
				...
				novacodeModel: openRouterDefaultModelId,
				novacodeToken: "novacode-token",
				// novacode_change end
			},
	...
	test("getState returns correct initial state", async () => {
		...
		// expect(state).toHaveProperty("taskHistory") // novacode_change
...
describe("Project MCP Settings", () => {
	...
	test.skip("handles openProjectMcpSettings message", async () => {
		...
		expect(mockedFs.mkdir).toHaveBeenCalledWith("/test/workspace/.novacode", { recursive: true })

		// Verify file was created with default content
		expect(safeWriteJson).toHaveBeenCalledWith("/test/workspace/.roo/mcp.json", { mcpServers: {} })

		// Check that openFile was called
		expect(openFileSpy).toHaveBeenCalledWith("/test/workspace/.novacode/mcp.json")
	})
	...
	test.skip("handles openProjectMcpSettings file creation error", async () => {
		...
		expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
			// novacode_change
			expect.stringContaining("Failed to create or open .novacode/mcp.json"),
		)
	})
})
...
describe("ClineProvider - Router Models", () => {
	...
	test("handles requestRouterModels with successful responses", async () => {
		...
		expect(getModels).toHaveBeenCalledWith({ provider: "openrouter", apiKey: "openrouter-key" }) // novacode_change: apiKey
		// novacode_change start
		...
		expect(getModels).toHaveBeenCalledWith({ provider: "glama" }) // novacode_change
	...
	test("handles requestRouterModels with individual provider failures", async () => {
		...
		vi.spyOn(provider, "getState").mockResolvedValue({
			apiConfiguration: {
				...
				geminiApiKey: "gemini-key",
				googleGeminiBaseUrl: "https://gemini.example.com",
				nanoGptApiKey: "nano-gpt-key", // novacode_change
				ovhCloudAiEndpointsApiKey: "ovhcloud-key",
				inceptionLabsApiKey: "inception-key",
				inceptionLabsBaseUrl: "https://api.inceptionlabs.ai/v1/",
		...
		vi.mocked(getModels)
			...
			.mockResolvedValueOnce(mockModels) // novacode_change: gemini success
			.mockRejectedValueOnce(new Error("Requesty API error")) //
			.mockResolvedValueOnce(mockModels) // novacode_change glama success
			.mockRejectedValueOnce(new Error("Unbound API error")) // unbound fail
			.mockRejectedValueOnce(new Error("Novacode-OpenRouter API error")) // novacode-openrouter fail
			...
			.mockResolvedValueOnce(mockModels) // nano-gpt success // novacode_change
			.mockResolvedValueOnce(mockModels) // novacode_change: aihubmix
			.mockResolvedValueOnce(mockModels) // novacode_change: ovhcloud
			.mockResolvedValueOnce(mockModels) // novacode_change: inception success
			.mockResolvedValueOnce(mockModels) // novacode_change: synthetic success
			.mockResolvedValueOnce(mockModels) // roo success
			.mockRejectedValueOnce(new Error("Chutes API error")) // chutes fail
			.mockResolvedValueOnce(mockModels) // novacode_change: poe success
		...
		expect(mockPostMessage).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				values: expect.objectContaining({ provider: "requesty" }),
			}),
		)
		...
		expect(mockPostMessage).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				values: expect.objectContaining({ provider: "unbound" }),
			}),
		)
		...
		expect(mockPostMessage).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				values: expect.objectContaining({ provider: "chutes" }),
			}),
		)
		// novacode_change end
		...
		expect(mockPostMessage).toHaveBeenCalledWith(
			expect.objectContaining({
				type: "singleRouterModelFetchResponse",
				success: false,
				error: "Novacode-OpenRouter API error",
				values: expect.objectContaining({ provider: "novacode" }),
			}),
		)
		...
		expect(mockPostMessage).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				values: expect.objectContaining({ provider: "litellm" }),
			}),
		)
	})
