...
describe("generateErrorDiagnostics", () => {
	...
	it("generates a diagnostics file with error metadata and history", async () => {
		...
		expect(result.filePath).toContain("nova-diagnostics-")
		...
		expect(String(writtenPath)).toContain("nova-diagnostics-test-tas")
		expect(String(writtenContent)).toContain(
			"// Please share this file with Nova Code Support (support@nova.ai) to diagnose the issue faster",
		)
