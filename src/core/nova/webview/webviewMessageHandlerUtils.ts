...
interface FetchNotificationsResult {
	...
	nativeNotifications: NovacodeNotification[]
}

/**
 * Core function to fetch Novacode notifications from the API
 * Can be used both from webview handler and standalone startup
 */
export async function fetchNovacodeNotificationsCore(
	...
	const url = getNovaUrlFromToken("https://api.nova.ai/api/users/notifications", novacodeToken)
	const response = await axios.get(url, {
		headers,
		timeout: 5000,
	})

	const notifications: NovacodeNotification[] = response.data?.notifications || []
...
/**
 * Display native VSCode notifications for Novacode notifications
 */
...
/**
 * Fetch and display Novacode notifications on extension startup
 * This function works without requiring the webview to be open
 */
export async function fetchNovacodeNotificationsOnStartup(
	...
	try {
		...
		if (!novacodeToken || apiConfiguration?.apiProvider !== "novacode") {
			log?.("[Notifications] Skipping notification fetch: not using novacode provider")
			return
		}

		log?.("[Notifications] Fetching notifications on startup...")

		const { nativeNotifications } = await fetchNovacodeNotificationsCore(
			novacodeToken,
...
const resendMessageSequence = async (
	...
	if (!currentCline || currentCline.taskId !== taskId) {
		...
		vscode.window.showErrorMessage(t("novacode:userFeedback.message_update_failed"))
		return false
	}
...
export const fetchNovacodeNotificationsHandler = async (provider: ClineProvider) => {
	try {
		...
		const { notifications, nativeNotifications } = await fetchNovacodeNotificationsCore(
			novacodeToken,
			dismissedNotificationIds || [],
			apiConfiguration.novacodeTesterWarningsDisabledUntil,
		...
		provider.log(`Error fetching Novacode notifications: ${error.message}`)
		provider.postMessageToWebview({
			type: "novacodeNotificationsResponse",
			notifications: [],
		})
	...
	try {
		...
		if (revert) {
			...
			if (checkpointMessage && checkpointMessage.text) {
				...
				vscode.window.showErrorMessage(t("novacode:userFeedback.no_checkpoint_found"))
			}
		}
		...
		if (success) {
			vscode.window.showInformationMessage(t("novacode:userFeedback.message_updated"))
		}
	} catch (error) {
				...
				try {
					if (profileName) {
						...
						await provider.upsertProviderProfile(
							...
							true, // Activate immediately to match old handleNovaCodeCallback behavior
						)
					} else {
