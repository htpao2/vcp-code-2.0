 ...
 * stored in {projectRoot}/.novacode/worktrees/
 */
...
export interface WorktreeInfo {
	...
	parentBranch: string
	createdAt: number
	sessionId?: string // Session ID from .novacode/session-id file, if present
}
...
export function generateBranchName(prompt: string): string {
	...
	return `${sanitized || "nova"}-${timestamp}`
}

const NOVACODE_DIR = ".novacode"
...
export class WorktreeManager {
	...
	constructor(projectRoot: string, outputChannel: vscode.OutputChannel) {
		...
		this.worktreesDir = path.join(projectRoot, NOVACODE_DIR, "worktrees")
	...
	/**
	 * Discover existing worktrees in .novacode/worktrees/
	 */
	...
	/**
	 * Write a session ID to the worktree's .novacode/session-id file.
	...
	async writeSessionId(worktreePath: string, sessionId: string): Promise<void> {
		const novacodeDir = path.join(worktreePath, NOVACODE_DIR)
		const sessionIdPath = path.join(novacodeDir, SESSION_ID_FILE)

		// Ensure .novacode directory exists in the worktree
		if (!fs.existsSync(novacodeDir)) {
			await fs.promises.mkdir(novacodeDir, { recursive: true })
		}

		await fs.promises.writeFile(sessionIdPath, sessionId, "utf-8")
		this.log(`Wrote session ID ${sessionId} to ${sessionIdPath}`)

		// Ensure .novacode/ is excluded from git in the worktree
		await this.ensureWorktreeGitExclude(worktreePath)
	}

	/**
	 * Read the session ID from a worktree's .novacode/session-id file.
	...
	async readSessionId(worktreePath: string): Promise<string | undefined> {
		const sessionIdPath = path.join(worktreePath, NOVACODE_DIR, SESSION_ID_FILE)
	...
	/**
	 * Ensure .novacode/ directory is excluded from git within a worktree.
	...
	/**
	 * Ensure .novacode/worktrees/ directory exists
	 */
	...
	async ensureGitExclude(): Promise<void> {
		...
		const addition = content.endsWith("\n") || content === "" ? "" : "\n"
		const excludeEntry = `${addition}\n# Nova Code agent worktrees\n${entry}\n`

		await fs.promises.appendFile(excludePath, excludeEntry)
		this.log("Added .novacode/worktrees/ to .git/info/exclude")
	}
