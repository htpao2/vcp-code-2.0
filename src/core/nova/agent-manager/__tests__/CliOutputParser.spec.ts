...
describe("parseCliChunk", () => {
	...
	it("should parse welcome event with configuration error instructions", () => {
		const result = parseCliChunk(
			'{"type":"welcome","metadata":{"welcomeOptions":{"instructions":["Configuration Error: config.json is incomplete","novacodeToken is required"]}},"timestamp":1234567890}\n',
		)
		expect(result.events).toHaveLength(1)
		expect(result.events[0]).toEqual({
			streamEventType: "welcome",
			worktreeBranch: undefined,
			timestamp: 1234567890,
			instructions: ["Configuration Error: config.json is incomplete", "novacodeToken is required"],
		})
	})
	...
	it("should parse JSON with VT control codes", () => {
		...
		const result = parseCliChunk(input)
		expect(result.events).toHaveLength(1)
		expect(result.events[0]).toMatchObject({ streamEventType: "novacode", payload: { timestamp: 123 } })
	})
...
describe("CliOutputParser class", () => {
	...
	it("should handle streaming chunks correctly", () => {
		...
		expect(events[0]).toMatchObject({ streamEventType: "novacode", payload: { content: "Hello" } })
		expect(events[1]).toMatchObject({ streamEventType: "novacode", payload: { content: "Hello World" } })
