...
describe("WorktreeManager", () => {
	...
	describe("createWorktree", () => {
		...
		it("creates worktree with generated branch name from prompt", async () => {
			...
			expect(containsPathSegments(result.path, ".novacode", "worktrees")).toBe(true)
		...
		it("removes existing worktree directory before creating", async () => {
			...
			vi.mocked(fs.existsSync).mockImplementation((p) => {
				...
				return normalized.endsWith(".git") || normalized.includes(".novacode/worktrees")
			})
			vi.mocked(fs.promises.stat).mockResolvedValue({ isDirectory: () => true, isFile: () => false } as any)
			vi.mocked(fs.promises.mkdir).mockResolvedValue(undefined)
			vi.mocked(fs.promises.rm).mockResolvedValue(undefined)
			vi.mocked(fs.promises.readFile).mockResolvedValue(".novacode/worktrees/")
			vi.mocked(fs.promises.appendFile).mockResolvedValue(undefined)
			...
			expect(containsPathSegments(String(rmCall[0]), ".novacode", "worktrees")).toBe(true)
		})
	})
	...
	describe("ensureGitExclude", () => {
		it("adds exclude entry when not present", async () => {
			...
			expect(fs.promises.appendFile).toHaveBeenCalledWith(
				...
				expect.stringContaining(".novacode/worktrees/"),
			)
		})
	...
	describe("writeSessionId", () => {
		it("creates .novacode directory and writes session ID file", async () => {
			vi.mocked(fs.existsSync).mockReturnValue(false)
			vi.mocked(fs.promises.mkdir).mockResolvedValue(undefined)
			...
			expect(fs.promises.mkdir).toHaveBeenCalledWith(
				path.join("/worktree/path", ".novacode"),
				expect.objectContaining({ recursive: true }),
			)
			expect(fs.promises.writeFile).toHaveBeenCalledWith(
				path.join("/worktree/path", ".novacode", "session-id"),
				"session-123",
				"utf-8",
			)
		})

		it("skips mkdir for .novacode if directory already exists", async () => {
			vi.mocked(fs.existsSync).mockImplementation((p) => {
				const normalized = String(p).replace(/\\/g, "/")
				// .novacode exists, but git info dir doesn't
				return normalized.endsWith(".novacode")
			})
			vi.mocked(fs.promises.writeFile).mockResolvedValue(undefined)
			vi.mocked(fs.promises.readFile).mockResolvedValue("gitdir: /main/.git/worktrees/test\n")
			vi.mocked(fs.promises.appendFile).mockResolvedValue(undefined)
			...
			// mkdir should only be called for git info dir, not for .novacode
			const mkdirCalls = vi.mocked(fs.promises.mkdir).mock.calls
			const novacodeMkdirCalls = mkdirCalls.filter(
				(call) =>
					String(call[0]).replace(/\\/g, "/").includes(".novacode") &&
					!String(call[0]).replace(/\\/g, "/").includes(".git"),
			)
			expect(novacodeMkdirCalls).toHaveLength(0)
		...
		it("ensures .novacode/ is excluded from git in the worktree", async () => {
			vi.mocked(fs.existsSync).mockReturnValue(false)
			vi.mocked(fs.promises.mkdir).mockResolvedValue(undefined)
			vi.mocked(fs.promises.writeFile).mockResolvedValue(undefined)
			vi.mocked(fs.promises.readFile).mockResolvedValue("gitdir: /main/.git/worktrees/test\n")
			...
			// Should have appended .novacode/ to the worktree's git exclude
			expect(fs.promises.appendFile).toHaveBeenCalledWith(
				expect.stringContaining("exclude"),
				expect.stringContaining(".novacode/"),
			)
		})
	})
	...
	describe("removeSessionId", () => {
		it("removes session ID file when it exists", async () => {
			...
			expect(fs.promises.unlink).toHaveBeenCalledWith(path.join("/worktree/path", ".novacode", "session-id"))
		})
