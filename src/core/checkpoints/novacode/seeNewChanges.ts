...
export async function seeNewChanges(task: Task, commitRange: CommitRange) {
	try {
		const service = await getCheckpointService(task)
		if (!service) {
			vscode.window.showWarningMessage(t("novacode:seeNewChanges.checkpointsUnavailable"))
			return
		}

		const changes = await service.getDiff(commitRange)
		if (changes.length === 0) {
			vscode.window.showWarningMessage(t("novacode:seeNewChanges.noChanges"))
			return
		}

		await vscode.commands.executeCommand(
			"vscode.changes",
			t("novacode:seeNewChanges.title"),
			changes.map((change) => [
		...
		vscode.window.showErrorMessage(t("novacode:seeNewChanges.error"))
