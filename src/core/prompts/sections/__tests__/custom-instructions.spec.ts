...
describe("loadRuleFiles", () => {
	...
	it("should not combine content from multiple rule files when they exist", async () => {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString().endsWith(".novacoderules")) {
	...
	it("should use .novacode/rules/ directory when it exists and has files", async () => {
		// Simulate .novacode/rules directory exists
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				parentPath: "/fake/path/.novacode/rules",
			},
			{
				name: "file2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			if (
				normalizedPath.includes("/fake/path/.novacode/rules/file1.txt") ||
				normalizedPath.includes("/fake/path/.novacode/rules/file2.txt")
			) {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.novacode/rules/file1.txt") {
				return Promise.resolve("content of file1")
			}
			if (normalizedPath === "/fake/path/.novacode/rules/file2.txt") {
		...
		const expectedRelativePath1 =
			process.platform === "win32" ? ".novacode\\rules\\file1.txt" : ".novacode/rules/file1.txt"
		const expectedRelativePath2 =
			process.platform === "win32" ? ".novacode\\rules\\file2.txt" : ".novacode/rules/file2.txt"
		...
		const expectedRulesDir =
			process.platform === "win32" ? "\\fake\\path\\.novacode\\rules" : "/fake/path/.novacode/rules"
		const expectedFile1Path =
			process.platform === "win32"
				? "\\fake\\path\\.novacode\\rules\\file1.txt"
				: "/fake/path/.novacode/rules/file1.txt"
		const expectedFile2Path =
			process.platform === "win32"
				? "\\fake\\path\\.novacode\\rules\\file2.txt"
				: "/fake/path/.novacode/rules/file2.txt"
	...
	it("should fall back to .novacoderules when .novacode/rules/ is empty", async () => {
		// Simulate .novacode/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
	...
	it("should read files from nested subdirectories in .novacode/rules/", async () => {
		// Simulate .novacode/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			...
			},
			{
				...
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.novacode/rules/subdir",
			},
			{
				name: "nested2.txt",
				isFile: () => true,
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.novacode/rules/subdir/subdir2",
			},
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.novacode/rules/root.txt") {
				return Promise.resolve("root file content")
			}
			if (normalizedPath === "/fake/path/.novacode/rules/subdir/nested1.txt") {
				return Promise.resolve("nested file 1 content")
			}
			if (normalizedPath === "/fake/path/.novacode/rules/subdir/subdir2/nested2.txt") {
		...
		const expectedRelativeRootPath =
			process.platform === "win32" ? ".novacode\\rules\\root.txt" : ".novacode/rules/root.txt"
		const expectedRelativeNested1Path =
			process.platform === "win32"
				? ".novacode\\rules\\subdir\\nested1.txt"
				: ".novacode/rules/subdir/nested1.txt"
		const expectedRelativeNested2Path =
			process.platform === "win32"
				? ".novacode\\rules\\subdir\\subdir2\\nested2.txt"
				: ".novacode/rules/subdir/subdir2/nested2.txt"
		...
		const expectedRootPath2 =
			process.platform === "win32"
				? "\\fake\\path\\.novacode\\rules\\root.txt"
				: "/fake/path/.novacode/rules/root.txt"
		const expectedNested1Path2 =
			process.platform === "win32"
				? "\\fake\\path\\.novacode\\rules\\subdir\\nested1.txt"
				: "/fake/path/.novacode/rules/subdir/nested1.txt"
		const expectedNested2Path2 =
			process.platform === "win32"
				? "\\fake\\path\\.novacode\\rules\\subdir\\subdir2\\nested2.txt"
				: "/fake/path/.novacode/rules/subdir/subdir2/nested2.txt"
...
describe("addCustomInstructions", () => {
	...
	it("should combine all instruction types when provided", async () => {
		// Simulate no .novacode/rules-test-mode directory
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
		...
		expect(result).toContain("Rules from .novacoderules-test-mode:\nmode specific rules")
	})
	...
	it("should use .novacode/rules-test-mode/ directory when it exists and has files", async () => {
		// Simulate .novacode/rules-test-mode directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				isFile: () => true,
				isSymbolicLink: () => false,
				parentPath: "/fake/path/.novacode/rules-test-mode",
			},
			{
				name: "rule2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			const normalizedPath = path.toString().replace(/\\/g, "/")
			if (
				normalizedPath.includes("/fake/path/.novacode/rules-test-mode/rule1.txt") ||
				normalizedPath.includes("/fake/path/.novacode/rules-test-mode/rule2.txt")
			) {
				return Promise.resolve({
					isFile: vi.fn().mockReturnValue(true),
				}) as any
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.novacode/rules-test-mode/rule1.txt") {
				return Promise.resolve("mode specific rule 1")
			}
			if (normalizedPath === "/fake/path/.novacode/rules-test-mode/rule2.txt") {
		...
		const expectedRelativeRule1Path =
			process.platform === "win32"
				? ".novacode\\rules-test-mode\\rule1.txt"
				: ".novacode/rules-test-mode/rule1.txt"
		const expectedRelativeRule2Path =
			process.platform === "win32"
				? ".novacode\\rules-test-mode\\rule2.txt"
				: ".novacode/rules-test-mode/rule2.txt"
		...
		const expectedAbsTestModeDir =
			process.platform === "win32"
				? "\\fake\\path\\.novacode\\rules-test-mode"
				: "/fake/path/.novacode/rules-test-mode"
		const expectedAbsRule1Path =
			process.platform === "win32"
				? "\\fake\\path\\.novacode\\rules-test-mode\\rule1.txt"
				: "/fake/path/.novacode/rules-test-mode/rule1.txt"
		const expectedAbsRule2Path =
			process.platform === "win32"
				? "\\fake\\path\\.novacode\\rules-test-mode\\rule2.txt"
				: "/fake/path/.novacode/rules-test-mode/rule2.txt"
	...
	it("should fall back to .novacoderules-test-mode when .novacode/rules-test-mode/ does not exist", async () => {
		// Simulate .novacode/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })

		// Simulate .novacoderules-test-mode exists
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString().includes(".novacoderules-test-mode")) {
	...
	it("should correctly format content from directories when using .novacode/rules-test-mode/", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "rule1.txt", isFile: () => true, parentPath: "/fake/path/.novacode/rules-test-mode" },
		] as any)
		readFileMock.mockReset()
...
describe("Rules directory reading", () => {
	it.skipIf(process.platform === "win32")("should follow symbolic links in the rules directory", async () => {
			...
			.mockResolvedValueOnce([
				{
					...
					isFile: () => true,
					parentPath: "/fake/path/.novacode/rules/symlink-target-dir",
				},
			] as any)
		...
		statMock.mockImplementation((path: string) => {
			// For directory check
			if (path === "/fake/path/.novacode/rules" || path.endsWith("dir")) {
				return Promise.resolve({
					isDirectory: vi.fn().mockReturnValue(true),
					isFile: vi.fn().mockReturnValue(false),
				} as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.novacode/rules/regular.txt") {
				return Promise.resolve("regular file content")
			}
			if (normalizedPath === "/fake/path/.novacode/symlink-target.txt") {
				return Promise.resolve("symlink target content")
			}
			if (normalizedPath === "/fake/path/.novacode/rules/symlink-target-dir/subdir_link.txt") {
				return Promise.resolve("regular file content under symlink target dir")
			}
			if (normalizedPath === "/fake/path/.novacode/nested-symlink-target.txt") {
		...
		const expectedRelativeRegularPath =
			process.platform === "win32" ? ".novacode\\rules\\regular.txt" : ".novacode/rules/regular.txt"
		const expectedRelativeSymlinkPath =
			process.platform === "win32" ? ".novacode\\symlink-target.txt" : ".novacode/symlink-target.txt"
		const expectedRelativeSubdirPath =
			process.platform === "win32"
				? ".novacode\\rules\\symlink-target-dir\\subdir_link.txt"
				: ".novacode/rules/symlink-target-dir/subdir_link.txt"
		const expectedRelativeNestedPath =
			process.platform === "win32"
				? ".novacode\\nested-symlink-target.txt"
				: ".novacode/nested-symlink-target.txt"
		...
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.novacode/rules/link.txt")
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.novacode/rules/link_dir")

		// Verify both files were read
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.novacode/rules/regular.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.novacode/symlink-target.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith(
			"/fake/path/.novacode/rules/symlink-target-dir/subdir_link.txt",
			"utf-8",
		)
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.novacode/nested-symlink-target.txt", "utf-8")
	})
	beforeEach(() => {
		vi.clearAllMocks()
	})
	...
	it.skipIf(process.platform === "win32")("should correctly format multiple files from directory", async () => {
		...
		// Simulate listing files
		readdirMock.mockResolvedValueOnce([
			{ name: "file1.txt", isFile: () => true, parentPath: "/fake/path/.novacode/rules" },
			{ name: "file2.txt", isFile: () => true, parentPath: "/fake/path/.novacode/rules" },
			{ name: "file3.txt", isFile: () => true, parentPath: "/fake/path/.novacode/rules" },
		] as any)
		...
		statMock.mockImplementation((path) => {
			...
			expect([
				"/fake/path/.novacode/rules/file1.txt",
				"/fake/path/.novacode/rules/file2.txt",
				"/fake/path/.novacode/rules/file3.txt",
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.novacode/rules/file3.txt") {
		...
		const expectedRelativeFile3Path =
			process.platform === "win32" ? ".novacode\\rules\\file3.txt" : ".novacode/rules/file3.txt"
