...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	// novacode_change
	return path.join(cwd, ".novacode", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .novacode/system-prompt-[mode slug]
...
/**
 * Ensures the .novacode directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	// novacode_change
	const rooDir = path.join(cwd, ".novacode")
