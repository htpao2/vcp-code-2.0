import { parseNovaSlashCommands } from "../nova"
import { ClineRulesToggles } from "../../../shared/cline-rules"
import fs from "fs/promises"
import path from "path"
import os from "os"

describe("parseNovaSlashCommands", () => {
	...
	describe("when no slash commands are present", () => {
		it("should return the original text unchanged", async () => {
			...
			const result = await parseNovaSlashCommands(text, emptyToggles, emptyToggles)
	...
	describe("workflow toggles", () => {
		...
		beforeEach(async () => {
			tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-test-"))
		...
		it("should process local workflow toggle commands", async () => {
			...
			const result = await parseNovaSlashCommands(text, localToggles, emptyToggles)
		...
		it("should process global workflow toggle commands", async () => {
			...
			const text = "<task>/my-workflow.md</task>"
			const result = await parseNovaSlashCommands(text, emptyToggles, globalToggles)
		...
		it("should prefer local workflow over global when both exist", async () => {
			...
			const result = await parseNovaSlashCommands(text, localToggles, globalToggles)
