...
describe("GitStateService", () => {
	...
	describe("getGitState", () => {
		...
		it("returns git state with repo URL, head, branch, and patch", async () => {
			const mockGit = {
				...
				raw: vi
					.fn()
					.mockResolvedValueOnce("refs/heads/main") // symbolic-ref for branch
		...
		it("falls back to origin when tracking remote points to non-existent remote", async () => {
			const mockGit = {
				...
				raw: vi
					.fn()
					.mockResolvedValueOnce("refs/heads/feature-branch") // symbolic-ref for branch
					.mockResolvedValueOnce("nonexistent") // config (remote doesn't exist in list)
					.mockResolvedValueOnce(""), // ls-files
				diff: vi.fn().mockResolvedValue("diff content"),
			}
			mockSimpleGit.mockReturnValue(mockGit as any)
		...
		it("handles detached HEAD - no branch", async () => {
			const mockGit = {
				...
				raw: vi
					.fn()
					.mockRejectedValueOnce(new Error("not a symbolic ref")) // symbolic-ref fails (detached HEAD)
					.mockResolvedValueOnce(""), // ls-files
				diff: vi.fn().mockResolvedValue("diff content"),
			}
			mockSimpleGit.mockReturnValue(mockGit as any)
	...
	describe("executeGitRestore", () => {
		...
		beforeEach(() => {
			...
			mockPathJoin.mockReturnValue("/tmp/novacode-git-patches/123.patch")
			mockMkdtempSync.mockReturnValue("/tmp/novacode-git-patches")
		})
		...
		it("applies patch from temp file", async () => {
			...
			expect(mockWriteFileSync).toHaveBeenCalledWith("/tmp/novacode-git-patches/123.patch", "patch content")
			expect(mockGit.applyPatch).toHaveBeenCalledWith("/tmp/novacode-git-patches/123.patch")
		})
		...
		it("cleans up temp file after patch", async () => {
			...
			expect(mockRmSync).toHaveBeenCalledWith("/tmp/novacode-git-patches", {
				recursive: true,
				force: true,
			})
		})
