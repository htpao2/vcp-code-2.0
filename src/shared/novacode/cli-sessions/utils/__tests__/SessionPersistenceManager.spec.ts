...
describe("SessionPersistenceManager", () => {
	...
	beforeEach(() => {
		...
		mockPathProvider = {
			getTasksDir: vi.fn().mockReturnValue("/home/user/.novacode/tasks"),
			getSessionFilePath: vi
				.fn()
				.mockImplementation((workspaceDir: string) => `${workspaceDir}/.novacode/session.json`),
		}
	...
	describe("setLastSession", () => {
		...
		it("should write last session to file", () => {
			...
			expect(mkdirSync).toHaveBeenCalledWith("/workspace/.novacode", { recursive: true })
			expect(writeFileSync).toHaveBeenCalledWith(
				"/workspace/.novacode/session.json",
