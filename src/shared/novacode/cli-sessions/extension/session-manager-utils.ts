...
import { ExtensionLoggerAdapter } from "../../../../services/nova-session/ExtensionLoggerAdapter"
import { ExtensionMessengerImpl } from "../../../../services/nova-session/ExtensionMessengerImpl"
import { ExtensionPathProvider } from "../../../../services/nova-session/ExtensionPathProvider"
import { SessionManager } from "../core/SessionManager"
import { buildApiHandler } from "../../../../api"
import * as vscode from "vscode"

const nova_isCli = () => {
	return process.env.NOVA_CLI_MODE === "true"
}

export async function nova_execIfExtension<T extends (...args: any) => any>(cb: T): Promise<ReturnType<T> | void> {
	if (nova_isCli()) {
...
export function nova_initializeSessionManager({
	novaToken,
	context,
	log,
	outputChannel,
	...
	return nova_execIfExtension(() => {
		try {
			...
			const sessionManager = SessionManager.init({
				...
				logger,
				extensionMessenger,
				getToken: () => Promise.resolve(novaToken),
					...
					const result = await (async () => {
						try {
							...
							if (currentTask?.taskId === taskId) {
								return currentTask.apiConfiguration.novacodeOrganizationId
							}

							const state = await provider.getState()

							return state.apiConfiguration.novacodeOrganizationId
						} catch {
							return undefined
						}
					})()
