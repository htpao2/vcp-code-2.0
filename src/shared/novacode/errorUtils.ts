...
/**
 * Error message thrown when the NovaCode token is missing or invalid.
 */
export const NOVACODE_TOKEN_REQUIRED_ERROR = "NovaCode token + baseUrl is required to fetch models"
...
export function isModelNotAllowedForTeamError(error: any) {
	//https://github.com/Nova-Org/novacode-backend/blob/66489e1b0f6f996338acf8bcc1b3558252e20e9d/src/app/api/openrouter/%5B...path%5D/route.ts#L371
	return !!(error && error.status === 404 && (error.message?.indexOf("not allowed for your team") ?? -1) >= 0)
}

export function isAnyRecognizedNovaCodeError(error: any) {
	return (
